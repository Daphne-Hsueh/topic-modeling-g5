...
# Suppress standard UMAP warnings for clean terminal output
...
print(f"Loaded {len(embeddings)} embeddings. Starting Grid Search...\n")
# ==========================================
...
min_cluster_size_list = [30, 50, 80, 120]

results = []

print(f"\n{'n_neighbors':<12} | {'min_cluster':<12} | {'Topics':<8} | {'Noise %':<8} | {'NPMI Coherence':<15}")
print("-" * 65)

for nn, mcs in itertools.product(n_neighbors_list, min_cluster_size_list):
    
    reduced_emb = umap.UMAP(n_neighbors=nn, n_components=5, min_dist=0.0, metric='cosine', random_state=42).fit_transform(embeddings)
    clusterer = hdbscan.HDBSCAN(min_cluster_size=mcs, metric='euclidean', cluster_selection_method='eom').fit(reduced_emb)
    ...
    print(f"{nn:<12} | {mcs:<12} | {n_topics:<8} | {noise_ratio*100:<7.1f}% | {npmi_score:<15.4f}")
...
# Filter for reasonable guardrails on a 14k corpus
